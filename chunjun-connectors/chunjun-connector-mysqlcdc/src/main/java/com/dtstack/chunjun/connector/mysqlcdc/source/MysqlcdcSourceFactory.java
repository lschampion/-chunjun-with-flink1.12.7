...
public class MysqlcdcSourceFactory extends SourceFactory {
    ...
    public DataStream<RowData> createSource() {
        ...
        //        TableUtil.createRowType(Arrays.asList(columnNameMap),
        // Arrays.asList(columnTypeMap), getRawTypeConverter());

        // TODO: lisai 判断是否是"*",else 中是原始内容
        if (syncConf.getReader().getFieldList().size() == 1
                && "*".equals(syncConf.getReader().getFieldList().get(0).getName())) {
            LOG.info("Cdc Source use full columns for reason of \"*\")");
            CdcMetaInfo cdcMetaInfo = this.cdcMetaInfos.get(0);
            // 使用已经选择的列
            cdcMetaInfo
                    .getColumnMetaSelected()
                    .forEach(
                            col -> {
        ...
        DebeziumSourceFunction<RowData> mySqlSource =
                new MySQLSource.Builder<RowData>()
                        ...
                        .databaseList(
                                Lists.newArrayList(cdcConf.getDatabase())
                                        .toArray(new String[0])) // set captured database
                        .tableList(
                                cdcConf.getTableList()
                                        .toArray(new String[cdcConf.getSchemaList().size()]))
        ...
        // "MysqlCdcSource");
        DataStreamSource<RowData> mysqlCdcSource = env.addSource(mySqlSource, "MysqlCdcSource");
    ...
    private void initTableMetaInfos(CdcConf cdcConf, EDatabaseType databaseType) {
        String[] fieldArr =
                syncConf.getReader().getFieldList().stream()
        ...
        List<CdcMetaInfo> infos =
                cdcConf.getTableList().stream()
                        .map(
                                tableOrigin -> {
                                    String[] fullName = tableOrigin.split("\\.");
                                    String databaseOrSchemaName =
                                            fullName.length == 2 ? fullName[0] : "";
                                    String tableName =
                                            fullName.length == 2 ? fullName[1] : tableOrigin;
    ...
    private DebeziumDeserializationSchema<RowData> buildRowDataDebeziumDeserializeSchema(
        ...
        MysqlRowDataDebeziumDeserializeSchema schema =
                new MysqlRowDataDebeziumDeserializeSchema(
        ...
        //        MysqlCdcColumnTypeConverter converter =
        //                new MysqlCdcColumnTypeConverter(getTypeInformation());
        //        return converter;
        //        return null;
    }
