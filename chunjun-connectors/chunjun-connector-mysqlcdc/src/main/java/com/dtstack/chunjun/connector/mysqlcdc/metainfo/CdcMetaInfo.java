...
public class CdcMetaInfo {
        ...
        switch (databaseType) {
            ...
            default:
                throw new UnsupportedOperationException(
                        "unsupported database type:" + databaseType.name());
        }
    }
    ...
    public void buildMetaData(String database, String schema, String table) {
        ...
        //        } else {
        //            fieldList = null;
        //            throw new IllegalArgumentException("when field is filled,only one table is
        // permitted!");
        //        }
        //        List<MutableTriple<String,String, String>> tableIdentifyList =
        // tableList.stream().distinct().map(table -> {
        ...
        this.jdbcUrl =
                StringUtils.isEmpty(this.jdbcUrl) ? getDefaultJdbcUrl(this.database) : this.jdbcUrl;
        ...
        Pair<List<String>, List<String>> tableMetaData =
                JdbcUtil.getTableMetaData(database, schema, table, connection);
